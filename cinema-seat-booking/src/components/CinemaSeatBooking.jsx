...
const CinemaSeatBooking = (
    ...
    const getSeatType = (row) => {
        ...
        for (let i = 0; i < seatTypeEntries.length; i++) {
            let [type, config] = seatTypeEntries[i];
        ...
        // Fallback: If user does not have provided any seat type entries
    ...
    const getSeatClassName = (seat) => {
        const baseClass = `w-8 h-8 sm:w-10 sm:h-10 lg:w:12 h-12 m-1 rounded-t-lg border-2 cursor-pointer 
        ...
        if (seat.selected) {
            return `${baseClass} bg-green-500 border-green-600 text-white transform-all scale-110`
        }
    ...
    const uniqueSeatTypes = Object.entries(seatTypes).map(([type, config], index) => {
        return { type, color: colors[index % colors.length], ...config }
    });
    ...
    return (
        <div className='w-full min-h-screen bg-white-50 p-4'>
            {/* title  */}
            <div className='max-w-6xl mx-auto bg-white rounded-lg shadow-lg p-6'>
                    ...
                    <div className='flex flex-col items-center min-w-max'>
                        {
                            seats.map((row, rowIndex) => {
                                return <div key={rowIndex} className='flex items-center mb-2'>
                ...
                {/* legend  */}
                <div className='flex flex-wrap justify-center gap-6 mb-6 p-4 bg-gray-50 rounded-es-lg'>
                    {
                    ...
                    <div className='flex items-center'>
                        <div className='w-6 h-6 border-2 rounded-t-lg mr-2 bg-gray-500 border-gray-600'></div>
                        <span>Booked</span>
                    </div>
                </div>
                ...
                <button onClick={handleBooking}
                    ...
                    className={`w-full px-3 py-6 rounded-lg font-bold text-lg transition all duration-200
                ${selectedSeats.length > 0
